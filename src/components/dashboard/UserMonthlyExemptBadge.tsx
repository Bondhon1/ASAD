...
interface UserMonthlyExemptBadgeProps {
  reason?: string | null;
  className?: string;
}

/**
 * Shows a premium diamond icon next to a user's name when they are exempt
 * from monthly donations. Hover reveals a tooltip with the exemption reason.
 */
export default function UserMonthlyExemptBadge({
  reason,
  className = "",
}: UserMonthlyExemptBadgeProps) {
    ...
    >
      ...
      {hovered && (
        ...
        >
          ...
          {reason && (
            <p className="text-xs text-violet-600 bg-violet-50 rounded px-2 py-1">{reason}</p>
          )}
        </div>
      )}
    </div>
