...
export default function MonthlyPaymentWidget() {
  ...
  const overdueMonthsOnly = currentMonthSummary
    ? unpaidMonths.filter(
        (m: MonthSummary) => !(m.month === currentMonthSummary.month && m.year === currentMonthSummary.year)
      )
    : unpaidMonths;
  const overdueTotal = overdueMonthsOnly.reduce((s: number, m: MonthSummary) => s + m.dueAmount, 0);
    ...
    <>
      ...
      {submitTarget && (
        <MonthlyPaymentSubmitModal
          ...
          overdueMonths={overdueMonthsOnly.filter(m => !(m.month === submitTarget.month && m.year === submitTarget.year))}
